...
public class SpringSecurityConfig extends WebSecurityConfigurerAdapter {
	...
	public void configure(HttpSecurity http) throws Exception {
		http.csrf().disable().cors().and().authorizeHttpRequests().antMatchers(HttpMethod.POST, "/login").permitAll()
				...
				.hasAnyRole("ADMIN", "TUTOR", "ALUMNO", "PROFESOR").antMatchers("/tareas/**").hasAnyRole("ADMIN", "ALUMNO", "PROFESOR","TUTOR").and().userDetailsService(usuarioService).exceptionHandling()
